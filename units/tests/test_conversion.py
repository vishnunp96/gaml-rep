...
def test_valid_named_to_composed():
    ...
    assert str(one_hz_in_per_second) == '1.00 s^-1'
...
def test_valid_composed_to_composed():
    ...
    assert str(highway_kph) == '100.00 km h^-1'
    assert str(highway_mph) == '62.14 mi h^-1'

    assert within_epsilon(imp_vel(highway_kph),
                          highway_mph)

    assert str(imp_vel(highway_kph)) == '62.14 mi h^-1'
