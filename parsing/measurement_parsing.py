...
def generate_units_lark_regex():
	return ' | '.join(['\"'+regularize_unit(i)+'\"' for i in sorted(REGISTRY.keys(),reverse=True)])
def generate_prefixes_lark_regex():
	return ' | '.join(['\"'+regularize_unit(i)+'\"' for i in sorted(list(PREFIXES.keys())+['\micro'],reverse=True)])

__grammar_file = os.path.join(os.path.dirname(os.path.realpath(__file__)),'measurementgrammarwithsn.lark')

def get_lark_grammar():
	grammar = readFile(__grammar_file)
	grammar = grammar.replace('<UNITNAMES>', generate_units_lark_regex())
	grammar = grammar.replace('<UNITPREFIXES>', generate_prefixes_lark_regex())
	return grammar

textparser = Lark(get_lark_grammar(), parser="lalr", lexer="contextual", start='measurement')
#parser = Lark(readFile(__grammar_file), start='measurement')
unitparser = Lark(get_lark_grammar(), parser="lalr", lexer="contextual", start='units')

class MeasurementTransformerOld(Transformer):
	...
	def measurement(self, children):
		...
		if 'centralvalue' in values:
			return Measurement(
					...
					unit=values.get('units', DimensionlessUnit(1)),
...
if __name__ == "__main__":
	...
	for text in cases:
		...
		#print(f'{text} -> {parsed} -> {repr(parsed)}')
		#print(f'{str(text):<40}{str(parsed)}')
		print(f'{text}')
		...
		try:
			tree = textparser.parse(text)
			measurement = MeasurementTransformer().transform(tree)
			print(measurement)
