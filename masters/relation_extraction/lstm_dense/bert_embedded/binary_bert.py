...
from masters.relation_extraction.datasets.SpecificRelationBertDataset import SpecificRelationBertDataset
...
# Define model
class BertEmbedRelation_Binary(
    ...
    def __init__(self, window_pad, hidden, tokenizer: BertTokenizer, bert_model, relation_focus, entity_labels,
        ...
        super(BertEmbedRelation_Binary,
              self).__init__()
        ...
        input_features = 64 * 2 + self.entity_classes_num * 2 + 1
        self.dense = []
    ...
    def forward(self, x):
        ...
        sentence_entity_labels, _ = rnn.pad_packed_sequence(x[2], padding_value=0, batch_first=True)
        ...
        lstm_out = self.pool(total_embed)[1][0].permute(1, 2, 0).reshape(-1, 64 * 2)
        lstm_out = F.relu(lstm_out)
        # lstm_out, _ = rnn.pad_packed_sequence(lstm_out, batch_first=True, padding_value=0)

        entity_labels = rnn.pad_packed_sequence(x[3], padding_value=0, batch_first=True)[0]

        x = torch.cat([lstm_out, entity_labels, x[4].unsqueeze(1)], 1)
    ...
    def make_dataset(self, ann_list):
        return SpecificRelationBertDataset(ann_list, self.tokenizer, self.labels,
...
if __name__ == '__main__':
    ...
    for relation in relations:
        ...
        def make_model():
            ...
            #
            return BertEmbedRelation_Binary(args.window_pad, args.hidden,
                                            tokenizer=tokenizer,
