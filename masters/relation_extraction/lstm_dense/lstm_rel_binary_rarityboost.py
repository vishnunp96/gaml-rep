...
class IndexSplitSpansDirectedLabelledRelationModel_Binary(
    ...
    def __init__(self, window_pad, hidden, relation_focus, entity_labels,
        ...
        lstm_input_size = self.embedding.embedding_dim + self.entity_classes_num + 1
    ...
    def rarity_boost(self, idxs):
        ''' Boost the weights of rare tokens '''
        # idxs = batch_size * sequence_length
        weights = torch.zeros_like(idxs).float()
        for bi, batch in enumerate(idxs):
            occurrences = {}
            count = 0
            ...
            for i, idx in enumerate(batch):
                ...
                if val == 0:
                    continue
                weights[bi, i] = count / (occurrences.get(val, 0) + 1)
        weights = F.normalize(weights, dim=1)
        return weights

    def tfidf_weights(self, idxs):
        ''' Boost the weights of rare tokens with TFIDF '''
        # idxs = batch_size * sequence_length
        weights = torch.zeros_like(idxs).float()
        for bi, batch in enumerate(idxs):
            for i, idx in enumerate(batch):
                ...
                if val == 0:
                    continue
                word = self.rev_indexes.get(val, '')
                weights[bi, i] = self.tfidf.tfidf(word)
    ...
    def forward(self, x):
        all_idxs = []
        for span in x[:-2]:
            idxs, _ = rnn.pad_packed_sequence(span[0], padding_value=0,
                                              batch_first=True)
            all_idxs.append(idxs)
        all_idxs = torch.cat(all_idxs, 1)
        if self.tfidf is not None:
            idx_weights = self.tfidf_weights(all_idxs)
        else:
            idx_weights = self.rarity_boost(all_idxs)

        def process_span(spans, used):
            s, l = spans
            ...
            cur_weights = idx_weights[:, used:used + idxs.shape[1]].unsqueeze(2)
            s = torch.cat((s, l, cur_weights),
                          2)  # Concatenate entnaity labels # batch_size, sequence_length, embedding_dim + label_encoding + 1
            s = rnn.pack_padded_sequence(s, lengths, batch_first=True,
                                         enforce_sorted=False)
            s = self.pool(s)[1][0].permute(1, 2, 0).reshape(-1, 64 * 2)
            s = F.relu(s)
            return s, used+idxs.shape[1]


        x_list = []
        used = 0
        for xp in x[:-2]:
            x_item, used = process_span(xp, used)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-tf','--tfidf',default='none',help='TFIDF file path')
    ...
    rev_indexes = None
    if args.tfidf != 'none':
        from masters.TFIDF import TFIDF
        tfidf = TFIDF.load_dict_from_pickle(args.tfidf)
        rev_indexes = {v: k for k, v in embeddings.indexes.items()}
        rev_indexes[0] = ''
        rev_indexes[embeddings.fallback_index] = ''
    ...
    for relation in relations:
        ...
        def make_model():
            return IndexSplitSpansDirectedLabelledRelationModel_Binary(
                ...
                tfidf=tfidf, rev_indexes=rev_indexes,
