...
class SpecificRelationBertWindowedDataset(AnnotationDataset):
    def __init__(self, ann_list, tokenizer, relation_encoder,
        ...
        super(SpecificRelationBertWindowedDataset, self).__init__(ann_list, cuda)
        ...
        for a in self.anns:
            ...
            for start, end in itertools.permutations(a.entities, 2):
                if (start.type, end.type) in allowed_entity_pairs:
                    ...
                    boundaries = [] # can recover the windows from these indices
                    boundaries.extend([start_s - pre_window_start, start_e - pre_window_start])
                    boundaries.extend([end_s - pre_window_start, end_e - pre_window_start])
    ...
    def __getitem__(self, idx):
        ...
        taa = x[1].long()
        bs = x[2].long()
        el = torch.from_numpy(x[3]).long()
        ee = torch.from_numpy(x[4]).long()
        rev = torch.tensor(x[5])

        y = torch.tensor(y).long()

        return (tii, taa, bs, el, ee, rev), y

    def collate_fn(self, sequences):
        xs, ys = zip(*sequences)
        tii, taa, bs, el, ee, rev = tuple(zip(*xs))
        if self.cuda:
            return (rnn.pack_sequence(tii, False).cuda(),
                    rnn.pack_sequence(taa, False).cuda(),
                    rnn.pack_sequence(bs, False).cuda(),
