...
from gaml.annotations.datasets import RelationSplitSpansDirectedIndexesDataset

from gaml.annotations.models.base import BaseANNRelationModule

# Define model
class IndexSplitSpansDirectedRelationModel(BaseANNRelationModule):
	def __init__(self, window_pad, hidden, output_labels, entity_labels, allowed_relations, token_indexes, fallback_index, embedding=None, embedding_dim=None, num_embeddings=None):
		super(IndexSplitSpansDirectedRelationModel,self).__init__()
		...
		self.pool = nn.LSTM(self.embedding.embedding_dim, 64, bidirectional=True, batch_first=True) # Output 128 (64*2)
	...
	def make_dataset(self, ann_list):
		return RelationSplitSpansDirectedIndexesDataset(ann_list, self.token_indexes, self.fallback_index, self.labels, self.entity_labels, self.allowed_relations, self.window_pad, cuda=next(self.output_dense.parameters()).is_cuda)
	...
	def state_dict(self, destination=None, prefix='', keep_vars=False):
		_state_dict = super(IndexSplitSpansDirectedRelationModel, self).state_dict(destination=destination,prefix=prefix,keep_vars=keep_vars)
	...
	def load_from_state_dict(_state_dict):
		...
		model = IndexSplitSpansDirectedRelationModel(
...
if __name__ == '__main__':
	...
	def make_model():
		return IndexSplitSpansDirectedRelationModel(args.window_pad, args.hidden, output_labels, entity_labels, args.config.relations, embeddings.indexes, embeddings.fallback_index, embedding=torch.from_numpy(embeddings.values)).float().cuda()
	def make_opt(model):
