...
from gaml.annotations.datasets import EntityIndexesCharsDataset
...
# Define model
class IndexCharsWindowMemoryEntityModel(BaseANNEntityModule):
	def __init__(self, window_pad, hidden, char_embedding_dim, output_labels, token_indexes, fallback_index, word_embedding=None, word_embedding_shape=None):
		super(IndexCharsWindowMemoryEntityModel,self).__init__()
		...
		input_features = ((self.word_embedding.embedding_dim + 2*self.char_encoder.embedding_dim) * (self.window_pad*2 + 1)) + self.output_num * self.window_pad
		self.dense = []
	...
	def forward(self, inputs):
		...
		char_emb = F.pad(char_emb, (0,0,self.window_pad,self.window_pad,0,0),value=0.0) # (batch_size, sequence_length+2*window_pad, char_embedding_dim)
		...
		word_idxs_pad = F.pad(word_idxs,(self.window_pad,self.window_pad),value=0) # (batch_size, sequence_length+2*window_pad)
		word_emb = self.word_embedding(word_idxs_pad) # (batch_size, sequence_length+2*window_pad, word_embedding_dim)
		word_proj = torch.matmul(word_emb, self.word_projection) # Perform projection (batch_size, sequence_length+2*window_pad, word_embedding_dim)

		x = torch.cat((word_proj,char_emb), dim=2) # (batch_size, sequence_length+2*window_pad, word_embedding_dim+char_embedding_dim)

		y = torch.empty(word_idxs.shape[0], word_idxs.shape[1], self.output_num, device=x.device) # batch_size * sequence_length * output_num
		previous_output = self.initial_state.repeat(word_idxs.shape[0], 1).to(device=x.device) # batch_size * (output_num * window_pad)
		for i in range(self.window_pad, x.shape[1] - self.window_pad): # Over sequence_length dimension (ignoring padding)
			e_i = x[:,(i-self.window_pad):(i+self.window_pad+1),:] # batch_size * (window_pad*2+1) * total_embedding_dim
			e_i = e_i.reshape(word_idxs.shape[0], -1) # batch_size * ((window_pad*2+1)*total_embedding_dim)
	...
	def state_dict(self, destination=None, prefix='', keep_vars=False):
		_state_dict = super(IndexCharsWindowMemoryEntityModel, self).state_dict(destination=destination,prefix=prefix,keep_vars=keep_vars)
	...
	def load_from_state_dict(_state_dict):
		...
		model = IndexCharsWindowMemoryEntityModel(
...
if __name__ == '__main__':
	...
	stopwatch.logfile = os.path.join(args.modeldir, 'training_log.log')
	...
	if args.eval:
		model = IndexCharsWindowMemoryEntityModel.load_pretained(os.path.join(args.modeldir,modelname+'.pt'))
		...
		def make_model():
			return IndexCharsWindowMemoryEntityModel(args.window_pad, args.hidden, args.char_emb, output_labels, embeddings.indexes, embeddings.fallback_index, word_embedding=torch.from_numpy(embeddings.values)).float().cuda()
		def make_opt(model):
			adam_lr = 0.001
