...
# Define model
class StackedLSTMEntityModel(BaseANNEntityModule):
	def __init__(self, models, hidden_size, num_layers):
		super(StackedLSTMEntityModel,self).__init__()
		...
		self.lstm = nn.LSTM(self.input_features, self.hidden_size, self.num_layers, bidirectional=True, dropout=0.2)
	...
	def forward(self, xs):
		...
		# add argmax one-hot encoding as well
	...
	def state_dict(self, destination=None, prefix='', keep_vars=False):
		_state_dict = super(StackedLSTMEntityModel, self).state_dict(destination=destination,prefix=prefix,keep_vars=keep_vars)
...
if __name__ == '__main__':
	...
	parser.add_argument('--layers',type=int,default=1,help='Number of layers of LSTM cells.')
	...
	if args.eval:
		model = StackedLSTMEntityModel.load_pretained(os.path.join(args.modeldir,modelname+'.pt'))
		...
		def make_model():
			return StackedLSTMEntityModel(models, args.hidden, args.layers).float().cuda()
		def make_opt(model):
			adam_lr = 0.01 # 0.0001
