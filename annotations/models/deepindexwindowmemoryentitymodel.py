...
# Define model
class DeepIndexWindowMemoryEntityModel(BaseANNEntityModule):
	def __init__(self, window_pad, hidden, output_labels, token_indexes, fallback_index, embedding=None, embedding_dim=None, num_embeddings=None):
		super(DeepIndexWindowMemoryEntityModel,self).__init__()
	...
	def state_dict(self, destination=None, prefix='', keep_vars=False):
		_state_dict = super(DeepIndexWindowMemoryEntityModel, self).state_dict(destination=destination,prefix=prefix,keep_vars=keep_vars)
	...
	def load_from_state_dict(_state_dict):
		...
		model = DeepIndexWindowMemoryEntityModel(
...
if __name__ == '__main__':
	...
	def make_model():
		return DeepIndexWindowMemoryEntityModel(args.window_pad, args.hidden, output_labels, embeddings.indexes, embeddings.fallback_index, embedding=torch.from_numpy(embeddings.values)).float().cuda()
	def make_opt(model):
		adam_lr = 0.001
