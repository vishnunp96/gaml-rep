if __name__ == '__main__':
	...
	from gaml.utilities.torchutils import TemporalMinMaxMeanPooling,train,predict_from_dataloader
	...
	class BaselineRelationDataset(torch.utils.data.Dataset):
	...
	train_dataset = BaselineRelationDataset(ann_train)
	dev_dataset = BaselineRelationDataset(ann_dev)
	test_dataset = BaselineRelationDataset(ann_test)
	...
	# Define model
	class BaselineRelationModel(nn.Module):
		def __init__(self, embedding_dim, entity_classes_num, hidden, output_num):
			super(BaselineRelationModel,self).__init__()
			self.pool = TemporalMinMaxMeanPooling()
			self.dense1 = nn.Linear(embedding_dim*3*5 + entity_classes_num*2, hidden)
			self.output_dense = nn.Linear(hidden, output_num)

		def forward(self, x):

			x = tuple(self.pool(i) for i in x[:-1])+(x[-1],)
			x = torch.cat(x,1)
	...
	# Create model
	model = BaselineRelationModel(embeddings.dim, len(entity_labels.classes_), 32, output_num).float().cuda()
	print(model)
